...
export default async function handler(req) {
  try {
    ...
    const upstream = await fetch(`${piUrl}/api/create-order?${searchParams}`, {
    ...
    console.error('[proxy/create-order]', e.message);
    return Response.json({ success: false, error: 'unavailable' }, {
      status: 503,
